...
use FormglutMail\App\Services\Mailer\ValidatorTrait;
...
class BaseHandler
{
    ...
    {
        $this->app = $app ?: formglutMail();
        $this->manager = $manager ?: formglutMail(Manager::class);
    }
        ...
        if ($this->shouldBeLogged($status)) {
            ...
            if($this->existing_row_id) {
                ...
                if($row) {
                    ...
                    if(!$status) {
                        do_action('formglutmail_email_sending_failed_no_fallback', $row['id'], $this, $data);
                    }
                }
            } else {
                ...
                if(!$status) {
                    ...
                    do_action('formglutmail_email_sending_failed', $logId, $this, $data);
                }
            }
        }
    ...
    protected function shouldBeLogged($status)
    {
        ...
        if (defined('FORMGLUTMAIL_LOG_OFF') && FORMGLUTMAIL_LOG_OFF) {
            return false;
        }
        ...
        return apply_filters('formglutmail_will_log_email', $isLogOn, $miscSettings, $this);
    }
