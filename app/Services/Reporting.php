...
class Reporting
{
    ...
    public function getSendingStats($from, $to)
    {
        ...
        $tableName = $wpdb->prefix.FORMGLUT_MAIL_DB_PREFIX.'email_logs';
    ...
    {
        $select = [
            formglutMailDb()->raw('COUNT(id) AS count'),
            formglutMailDb()->raw('DATE('.$dateField.') AS date')
        ];

        if ($frequency == static::$weekly) {
            $select[] = formglutMailDb()->raw('WEEK(created_at) week');
        } else if ($frequency == static::$monthly) {
            $select[] = formglutMailDb()->raw('MONTH(created_at) month');
        }
