...
class TelegramController extends Controller
{
    public function issuePinCode(Request $request)
    {
        ...
        return $this->sendSuccess([
            'message'    => __('Awesome! Please activate the connection from your telegram account.', 'formglut-smtp'),
    ...
    public function confirmConnection(Request $request)
    {
        ...
        if (empty($siteToken)) {
            return $this->sendError([
                'message' => __('Please provide site token', 'formglut-smtp')
            ], 422);
        }
        ...
        update_option('_formglut_smtp_notify_settings', $previousSettings, false);

        return $this->sendSuccess([
            'success' => true,
            'message' => __('Connection successful', 'formglut-smtp'),
        ]);
    }
    ...
    public function getTelegramConnectionInfo(Request $request)
    {
        ...
        if (Arr::get($settings, 'telegram.status') != 'yes') {
            return $this->sendSuccess([
                'message'                => __('Telegram notification is not enabled', 'formglut-smtp'),
    ...
    public function disconnect()
    {
        ...
        return $this->sendSuccess([
            'message' => __('Telegram connection has been disconnected successfully', 'formglut-smtp')
        ]);
    }
}
