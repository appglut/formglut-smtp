...
class SettingsController extends Controller
{
    ...
    public function store(Request $request, Settings $settings, Factory $factory)
    {
        ...
        try {
            ...
            $data = apply_filters('formglutmail_saving_connection_data', $data, $data['connection']['provider']);
    ...
    public function storeMiscSettings(Request $request, Settings $settings)
    {
        ...
        $this->sendSuccess([
            'message' => __('General Settings has been updated', 'formglut-smtp')
        ]);
    }
    ...
    public function storeGlobals(Request $request, Settings $settings)
    {
        ...
        return $this->sendSuccess([
            'form'    => $data,
            'message' => __('Settings saved successfully.', 'formglut-smtp')
        ]);
    }
    ...
    public function sendTestEmil(Request $request, Settings $settings)
    {
        ...
        try {
            ...
            if (!isset($data['email'])) {
                return $this->sendError([
                    'email_error' => __('The email field is required.', 'formglut-smtp')
                ], 422);
            }

            if (!defined('FORMGLUTMAIL_EMAIL_TESTING')) {
                define('FORMGLUTMAIL_EMAIL_TESTING', true);
            }

            $settings->sendTestEmail($data, $settings->get());

            return $this->sendSuccess([
                'message' => __('Email delivered successfully.', 'formglut-smtp')
            ]);
    ...
    public function getConnectionInfo(Request $request, Settings $settings, Factory $factory)
    {
        ...
        if (!isset($connections[$connectionId]['provider_settings'])) {
            return $this->sendSuccess([
                'info' => __('Sorry no connection found. Please reload the page and try again', 'formglut-smtp')
            ]);
        }
    ...
    public function addNewSenderEmail(Request $request, Settings $settings, Factory $factory)
    {
        ...
        if (!is_email($email)) {
            return $this->sendError([
                'message' => __('Please provide a valid email address', 'formglut-smtp')
            ]);
        }
        ...
        return $this->sendSuccess([
            'message' => __('Email has been added successfully', 'formglut-smtp')
        ]);
    ...
    public function removeSenderEmail(Request $request, Settings $settings, Factory $factory)
    {
        ...
        return $this->sendSuccess([
            'message' => __('Email has been removed successfully', 'formglut-smtp')
        ]);
    ...
    public function installPlugin(Request $request)
    {
        ...
        $UrlMaps = [
            'formglutform'   => [
                'admin_url' => admin_url('admin.php?page=formglut_forms'),
                'title'     => __('Go to Formglut Forms Dashboard', 'formglut-smtp')
            ],
            'formglut-crm'   => [
                'admin_url' => admin_url('admin.php?page=formglutcrm-admin'),
                'title'     => __('Go to FormglutCRM Dashboard', 'formglut-smtp')
            ],
            'ninja-tables' => [
                'admin_url' => admin_url('admin.php?page=ninja_tables#/'),
                'title'     => __('Go to Ninja Tables Dashboard', 'formglut-smtp')
            ]
        ];

        if (!isset($UrlMaps[$pluginSlug]) || (defined('DISALLOW_FILE_MODS') && DISALLOW_FILE_MODS)) {
            $this->sendError([
                'message' => __('Sorry, You can not install this plugin', 'formglut-smtp')
            ]);
        }
        ...
        try {
            ...
            $this->send([
                'message' => __('Plugin has been successfully installed.', 'formglut-smtp'),
    ...
    public function subscribe()
    {
        ...
        if ($_REQUEST['share_essentials'] == 'yes') {
            update_option('_formglutsmtp_sub_update', 'shared', 'no');
            $shareEssentials = 'yes';
        } else {
            update_option('_formglutsmtp_sub_update', 'yes', 'no');
        }
    ...
    {
        $this->verify();
        update_option('_formglutsmtp_dismissed_timestamp', time(), 'no');
    ...
    private function pushData($optinEmail, $shareEssentials, $displayName = '')
    {
        ...
        $url = 'https://formglutsmtp.com/wp-admin/?formglutcrm=1&route=contact&hash=6012116c-90d8-42a5-a65b-3649aa34b356';
    ...
    public function getGmailAuthUrl(Request $request)
    {
        ...
        if (Arr::get($connection, 'key_store') == 'wp_config') {
            if (defined('FORMGLUTMAIL_GMAIL_CLIENT_ID')) {
                $clientId = FORMGLUTMAIL_GMAIL_CLIENT_ID;
            } else {
                return $this->sendError([
                    'client_id' => [
                        'required' => 'Please define FORMGLUTMAIL_GMAIL_CLIENT_ID in your wp-config.php file'
                    ]
                ]);
            }
            if (defined('FORMGLUTMAIL_GMAIL_CLIENT_SECRET')) {
                $clientSecret = FORMGLUTMAIL_GMAIL_CLIENT_SECRET;
            } else {
                return $this->sendError([
                    'client_secret' => [
                        'required' => 'Please define FORMGLUTMAIL_GMAIL_CLIENT_SECRET in your wp-config.php file'
                    ]
                ]);
            }
        }
        ...
        $authUrl = add_query_arg([
            ...
            'access_type'            => 'offline',
            'client_id'              => $clientId,
            'redirect_uri'           => apply_filters('formglutsmtp_gapi_callback', 'https://formglutsmtp.com/gapi/'),
            'state'                  => admin_url('admin.php?page=formglut_forms_smtp&gapi=1'),
    ...
    public function getOutlookAuthUrl(Request $request)
    {
        ...
        delete_option('_formglutsmtp_intended_outlook_info');

        if (Arr::get($connection, 'key_store') == 'wp_config') {
            if (defined('FORMGLUTMAIL_OUTLOOK_CLIENT_ID')) {
                $clientId = FORMGLUTMAIL_OUTLOOK_CLIENT_ID;
            } else {
                return $this->sendError([
                    'client_id' => [
                        'required' => 'Please define FORMGLUTMAIL_OUTLOOK_CLIENT_ID in your wp-config.php file'
                    ]
                ]);
            }
            if (defined('FORMGLUTMAIL_OUTLOOK_CLIENT_SECRET')) {
                $clientSecret = FORMGLUTMAIL_OUTLOOK_CLIENT_SECRET;
            } else {
                return $this->sendError([
                    'client_secret' => [
                        'required' => 'Please define FORMGLUTMAIL_OUTLOOK_CLIENT_SECRET in your wp-config.php file'
                    ]
                ]);
            }
        } else {
            update_option('_formglutsmtp_intended_outlook_info', [
                'client_id'     => $clientId,
        ...
        return $this->sendSuccess([
            'auth_url' => (new \FormglutMail\App\Services\Mailer\Providers\Outlook\API($clientId, $clientSecret))->getAuthUrl()
        ]);
    }
    ...
    public function saveNotificationSettings(Request $request)
    {
        ...
        update_option('_formglut_smtp_notify_settings', $settings, false);
