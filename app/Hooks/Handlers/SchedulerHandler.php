...
{
    protected $dailyActionName = 'formglutmail_do_daily_scheduled_tasks';

    public function register()
    {
        add_action($this->dailyActionName, array($this, 'handleScheduledJobs'));
        add_action('formglutmail_email_sending_failed', array($this, 'maybeHandleFallbackConnection'), 10, 3);

        add_action('formglutsmtp_renew_gmail_token', array($this, 'renewGmailToken'));

        add_action('formglutmail_email_sending_failed_no_fallback', array($this, 'maybeSendNotification'), 10, 3);
    ...
    private function deleteOldEmails()
    {
        ...
        if ($logSaveDays) {
            (new \FormglutMail\App\Models\Logger())->deleteLogsOlderThan($logSaveDays);
        }
    }
    ...
    public function sendDailyDigest()
    {
        ...
        $lastDigestSent = get_option('_formglutmail_last_email_digest');
        if ($lastDigestSent) {
        ...
        $emailBody = (string)formglutMail('view')->make('admin.digest_email', $data);
        $emailSubject = $reportingDate . ' email sending stats for ' . $this->getDomainName();

        $headers = array('Content-Type: text/html; charset=UTF-8');

        update_option('_formglutmail_last_email_digest', date('Y-m-d H:i:s'));

        return formglut_wp_mail($sendToArray, $emailSubject, $emailBody, $headers);
    ...
    public function maybeHandleFallbackConnection($logId, $handler, $data = [])
    {
        ...
        $settings = (new \FormglutMail\App\Models\Settings())->getSettings();

        $fallbackConnectionId = \FormglutMail\Includes\Support\Arr::get($settings, 'misc.fallback_connection');

        if (!$fallbackConnectionId) {
            do_action('formglutmail_email_sending_failed_no_fallback', $logId, $handler, $data);
            return false;
        }

        $fallbackConnection = \FormglutMail\Includes\Support\Arr::get($settings, 'connections.' . $fallbackConnectionId);
