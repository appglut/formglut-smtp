...
namespace FormglutSmtpLib\GuzzleHttp\Handler;

use FormglutSmtpLib\GuzzleHttp\Exception\ConnectException;
use FormglutSmtpLib\GuzzleHttp\Exception\RequestException;
use FormglutSmtpLib\GuzzleHttp\Promise\FulfilledPromise;
use FormglutSmtpLib\GuzzleHttp\Promise\PromiseInterface;
use FormglutSmtpLib\GuzzleHttp\Psr7;
use FormglutSmtpLib\GuzzleHttp\TransferStats;
use FormglutSmtpLib\GuzzleHttp\Utils;
use FormglutSmtpLib\Psr\Http\Message\RequestInterface;
use FormglutSmtpLib\Psr\Http\Message\ResponseInterface;
use FormglutSmtpLib\Psr\Http\Message\StreamInterface;
/**
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \FormglutSmtpLib\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \FormglutSmtpLib\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \FormglutSmtpLib\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \FormglutSmtpLib\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\FormglutSmtpLib\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \FormglutSmtpLib\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \FormglutSmtpLib\GuzzleHttp\debug_resource($value);
