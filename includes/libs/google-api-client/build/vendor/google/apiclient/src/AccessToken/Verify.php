 ...
 */
namespace FormglutSmtpLib\Google\AccessToken;

use DateTime;
use DomainException;
use Exception;
use FormglutSmtpLib\ExpiredException;
use FormglutSmtpLib\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use FormglutSmtpLib\Firebase\JWT\Key;
use FormglutSmtpLib\Firebase\JWT\SignatureInvalidException;
use FormglutSmtpLib\Google\Auth\Cache\MemoryCacheItemPool;
use FormglutSmtpLib\Google\Exception as GoogleException;
use FormglutSmtpLib\GuzzleHttp\Client;
use FormglutSmtpLib\GuzzleHttp\ClientInterface;
use InvalidArgumentException;
use LogicException;
use FormglutSmtpLib\phpseclib3\Crypt\PublicKeyLoader;
use FormglutSmtpLib\phpseclib3\Crypt\RSA\PublicKey;
// Firebase v2
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('FormglutSmtpLib\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'FormglutSmtpLib\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('FormglutSmtpLib\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('FormglutSmtpLib\\phpseclib3\\Crypt\\RSA')) {
            return 'FormglutSmtpLib\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('FormglutSmtpLib\\phpseclib\\Crypt\\RSA')) {
            return 'FormglutSmtpLib\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('FormglutSmtpLib\\phpseclib3\\Math\\BigInteger')) {
            return 'FormglutSmtpLib\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('FormglutSmtpLib\\phpseclib\\Math\\BigInteger')) {
            return 'FormglutSmtpLib\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('FormglutSmtpLib\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('FormglutSmtpLib\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('FormglutSmtpLib\\Crypt_RSA')) {
