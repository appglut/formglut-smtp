 ...
 */
namespace FormglutSmtpLib\Google\Auth;

use DateTime;
use Exception;
use FormglutSmtpLib\Firebase\JWT\ExpiredException;
use FormglutSmtpLib\Firebase\JWT\JWT;
use FormglutSmtpLib\Firebase\JWT\SignatureInvalidException;
use FormglutSmtpLib\Google\Auth\Cache\MemoryCacheItemPool;
use FormglutSmtpLib\Google\Auth\HttpHandler\HttpClientCache;
use FormglutSmtpLib\Google\Auth\HttpHandler\HttpHandlerFactory;
use FormglutSmtpLib\GuzzleHttp\Psr7\Request;
use FormglutSmtpLib\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use FormglutSmtpLib\phpseclib\Crypt\RSA;
use FormglutSmtpLib\phpseclib\Math\BigInteger;
use FormglutSmtpLib\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use FormglutSmtpLib\SimpleJWT\InvalidTokenException;
use FormglutSmtpLib\SimpleJWT\JWT as SimpleJWT;
use FormglutSmtpLib\SimpleJWT\Keys\KeyFactory;
use FormglutSmtpLib\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\FormglutSmtpLib\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\FormglutSmtpLib\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('FormglutSmtpLib\\SimpleJWT\\JWT')) {
    ...
    {
        $class = \class_exists('FormglutSmtpLib\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
