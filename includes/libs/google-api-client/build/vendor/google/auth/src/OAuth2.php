...
use FormglutSmtpLib\GuzzleHttp\Psr7\Query;
...
use FormglutSmtpLib\Psr\Http\Message\UriInterface;
/**
...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('FormglutSmtpLib\\Firebase\\JWT\\JWT')) {
            return \FormglutSmtpLib\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \FormglutSmtpLib\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('FormglutSmtpLib\\Firebase\\JWT\\JWT')) {
            return \FormglutSmtpLib\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \FormglutSmtpLib\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
